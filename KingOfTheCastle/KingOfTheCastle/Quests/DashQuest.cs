...
namespace KingOfTheCastle
{
    class DashQuest:Quest
    {
        ...
        public DashQuest(KingOfTheCastle game)
        {
            ...
            {
                playerCompletionProgress[i] = "0/" + requiredDashAmt;
            }
                ...
                foreach (Player p in game.players)
                {
                    if (p != null)
                    {
                        if (!p.completedMainQuest)
                            playerCompletionProgress[Array.IndexOf(game.players, p)] = p.dashes + "/" + requiredDashAmt;

                        if (p.dashes >= requiredDashAmt && !p.completedMainQuest)
                        {
