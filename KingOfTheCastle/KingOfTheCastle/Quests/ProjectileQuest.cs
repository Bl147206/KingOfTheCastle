...
namespace KingOfTheCastle
{
    class ProjectileQuest:Quest
    {
        ...
        {
            this.game = game;
            requiredProjAmt = (int)Math.Round((double)(game.round) / 5 * Globals.rng.Next(5, 10));
            ...
            {
                playerCompletionProgress[i] = "0/" + requiredProjAmt;
            }
                ...
                foreach (Player p in game.players)
                {
                    if (p != null)
                    {
                        if (!p.completedMainQuest)
                            playerCompletionProgress[Array.IndexOf(game.players, p)] = p.projectilesLanded + "/" + requiredProjAmt;

                        if (p.projectilesLanded >= requiredProjAmt && !p.completedMainQuest)
                        {
